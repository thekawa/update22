...
exports.postLoginPage = async (req, res) => {
  ...
  const user = await querysql('SELECT userID, firstname, lastname, email, password, roles FROM user WHERE email = ?', email)
...
if(roles == "conducteur") {
  return res.redirect('/dashboardC');}
 else {
  return res.redirect('/dashboardU'); }
...
exports.postRegisterPage = async (req, res) => {
  ...
  try {
    ...
    await querysql('INSERT INTO user (firstname, lastname, email, password, roles) VALUES (?, ?, ?, ?, ?)', [firstname, lastname, email, hash, roles],
    (err, result) => {
      if(err) {
